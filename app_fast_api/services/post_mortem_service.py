...
class PostMortemService:
    ...
    def _serialize_post_mortem(self, pm) -> Dict[str, Any]:
        ...
        return {
            ...
            'detection_time': pm.detection_time,
            'response_time': pm.response_time,
            'resolution_time': pm.resolution_time,
            'summary': pm.summary,
            ...
            'timeline_events': json.loads(pm.timeline_events) if pm.timeline_events else [],
            'response_actions': json.loads(pm.response_actions) if pm.response_actions else [],
            'resolution_description': pm.resolution_description,
            'preventive_actions': json.loads(pm.preventive_actions) if pm.preventive_actions else [],
            'lessons_learned': pm.lessons_learned,
            'action_items': json.loads(pm.action_items) if pm.action_items else [],
            'author': pm.author,
            'reviewers': json.loads(pm.reviewers) if pm.reviewers else [],
            'contributors': json.loads(pm.contributors) if pm.contributors else [],
            'tags': json.loads(pm.tags) if pm.tags else [],
            'related_incidents': json.loads(pm.related_incidents) if pm.related_incidents else [],
            'external_links': json.loads(pm.external_links) if pm.external_links else [],
